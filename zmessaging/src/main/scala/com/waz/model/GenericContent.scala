...
//        p.size = preview.size
    ...
    def apply(asset: AssetData, preview: Option[AssetData] = None): Messages.Asset = returning(new Messages.Asset) { proto =>
    ...
    def apply(asset: RawAsset[General], preview: Option[Asset2[General]]): Messages.Asset = returning(new Messages.Asset) { proto =>
      proto.original = Original(asset)
      preview.foreach(p => proto.preview = Preview(p))
