...
import com.waz.service.assets2.AssetsStorageImpl.{AssetDao, Codec}
import com.waz.utils.TrimmingLruCache.Fixed
import com.waz.utils.wrappers.{DB, DBCursor}
import com.waz.utils.{CachedStorage2, CirceJSONSupport, DbStorage2, InMemoryStorage2, Storage2, TrimmingLruCache}
...
trait AssetsStorage extends Storage2[AssetId, Asset[General]]

class AssetsStorageImpl(context: Context, db: DB, ec: ExecutionContext)
  extends CachedStorage2[AssetId, Asset[General]](
  ...
  )(ec) with AssetsStorage

object AssetsStorageImpl {
  ...
  object AssetDao extends Dao[Asset[General], AssetId] with ColumnBuilders[Asset[General]] with StorageCodecs with CirceJSONSupport {
    ...
    override def apply(implicit cursor: DBCursor): Asset[General] = {
      Asset(Id, Token, Sha, Encryption, Source, Details, ConvId)
    }
  ...
  implicit val EncryptionCodec: Codec[Encryption, String] = new Codec[Encryption, String] {
    ...
    override def deserialize(value: String): Encryption = value match {
      ...
      case str if str.startsWith(AES_CBC_Prefix) =>
        AES_CBC_Encryption(AESKey(str.substring(0, AES_CBC_Prefix.length - 1)))
    }
  }
