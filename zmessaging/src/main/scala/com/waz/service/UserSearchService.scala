...
class UserSearchService(selfUserId:           UserId,
  ...
  private def filterForPartner(query: Filter, searchResults: IndexedSeq[UserData]): Future[IndexedSeq[UserData]] = {
    ...
    isPartner.flatMap {
      ...
      case false if teamId.isDefined =>
        ...
        knownUsers.map { knownUsersIds =>
          val res = searchResults.filter { u =>
            ...
            ZLog.verbose(s"SEARCH known users: $knownUsersIds")
            ZLog.verbose(s"SEARCH is in known users: ${knownUsersIds.contains(u.id)}")
            ZLog.verbose(s"SEARCH is different team: ${u.teamId != teamId}")
            ZLog.verbose(s"SEARCH is partner: ${u.isPartner(teamId)}")
            ZLog.verbose(s"SEARCH is exact search: ${u.handle.exists(_.exactMatchQuery(query))}")
      ...
      case _ =>
        ZLog.verbose(s"SEARCH filterForPartner($query, ${searchResults.map(_.getDisplayName)}) with a private user, no filtering")
        Future.successful(searchResults)
    }
  }
  ...
  def updateSearchResults(query: SearchQuery, results: Seq[UserSearchEntry]) = {
    ...
    for {
      ...
      filtered <- filterForPartner(filter, userData)
    ...
    query match {
      case Recommended(handle) if handle.length > 1 && !results.map(_.handle).exists(_.exactMatchQuery(handle)) =>
        debug(l"SEARCH exact match requested")
  ...
  def updateExactMatch(handle: Handle, userId: UserId) = {
    ...
    debug(l"SEARCH update exact match: $handle, $userId")
    usersStorage.get(userId).collect {
      case Some(user) =>
        debug(l"SEARCH received exact match: ${user.handle}")
  ...
  private def startNewSearch(query: SearchQuery): Signal[IndexedSeq[UserData]] = returning( queryCache.optSignal(query) ){ _ =>
    ZLog.verbose(s"SEARCH startNewSearch($query)")
    ...
    case Some(cached) => cached.entries match {
      ...
      case Some(ids) =>
        ...
        usersStorage.listSignal(ids).flatMap(res => Signal.future(filterForPartner(filter, res)))
    }
  }
